function dataArray = readDataAndHeader(path,name)

fullPathToFile = strcat(path,'/',name,'.head');

headerFile = fopen(fullPathToFile,'rt');

if headerFile == -1
    display(['Eror opening header file:' strcat(name,'.head')])
    return;
end
...
data = fopen(strcat(path,'/',name,'.data'),'r');

if (a_n_bytes == 2)
    eeg = fread(data,'int16');
elseif (a_n_bytes == 4)
    eeg = fread(data,'int32');
end

dataArray = reshape(eeg,[],a_n_chan);
dataArray = dataArray*a_conv_factor;
