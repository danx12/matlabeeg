classdef bin_file < handle
    ...
    methods
        function self = bin_file(file_name,head_name,nts_name)
            ...
            if ~exist('nts_name', 'var') || isempty(nts_name)
                self.a_nts_name = [];
            else
            ...
            if ~exist(self.a_nts_name, 'file')
                [str_pathstr, str_name,ext] = fileparts(self.a_file_name);
                
                idx_pat=findstr(str_pathstr,'pat_');
                
                idx_un=findstr(str_pathstr,filesep);
                
                idx_un=idx_un(find(idx_un>idx_pat));
                
                
                
                %str_name = strtok(str_name, '_');
                
                self.a_nts_name = fullfile(str_pathstr(1:idx_un(1)-1), [str_pathstr(idx_pat(end)+4:idx_un(1)-1) '.evts']);
                
%                 if ~exist(self.a_note_name, 'file')
%                     self.a_note_name = [self.a_file_name '.evts'];
%                     if ~exist(self.a_note_name, 'file')
%                         display('[mat_file] - ERROR: Events file does not exist!');
%                         return;
%                     end
%                 end
            end
        ...
        function get_header_info(self)
            ...
            while 1
                ...
                if strcmpi(str_Token, 'SAMPLE_BYTES') || strcmpi(str_Token, 'SAMPLE_BYTE')
...
%                     %a struct and append it to the 'a_note_struct' array
            ...
            k=1;
            aux = 0;
            while(1)%Interpret the .evts file
                
                val=fgetl(fd_ev);
                if k~=1
                    if val~=-1
                        tab=sprintf('\t');
                        idx_ig=findstr(val,tab);
                        
                        if (~isnan(val(1:idx_ig(1)-1)))
                            self.a_evt(aux).type=(val(1:idx_ig(1)-1));
                        else
                            disp ('Information NOT available (type of event)')
                        end
                        
                        if (~isnan(val(idx_ig(1):idx_ig(2))))
                            self.a_evt(aux).start_ts=val(idx_ig(1)+1:idx_ig(2)-1);
                        else
                            disp ('Information NOT available (EEG_on_sec)')
                        end
                        
                        
                        
                        if (~isnan(val(idx_ig(2):idx_ig(3))))
                            self.a_evt(aux).eeg_on_sec=str2double(val(idx_ig(2):idx_ig(3)));
                        else
                            disp ('Information NOT available (EEG_on_sec)')
                        end
                        
                        if (~isnan(str2double(val(idx_ig(6):idx_ig(7))))) && (str2double(val(idx_ig(6):idx_ig(7)))~=0)
                            self.a_evt(aux).eeg_off_sec = str2double(val(idx_ig(6):idx_ig(7)));
                        else
                            self.a_evt(aux).eeg_off_sec = self.a_evt(aux).eeg_on_sec+self.a_wsize; %default
                            disp ('Information NOT available (EEG_off_sec)')
                        end
                        
                        
                        
                        
                        if ((~strcmpi(val(idx_ig(5)+1:idx_ig(6)-1),'')) && (~strcmpi(val(idx_ig(5)+1:idx_ig(6)-1),'NULL')))
                            self.a_evt(aux).stop_ts = val(idx_ig(5)+1:idx_ig(6)-1);
                        else
                            self.a_evt(aux).stop_ts = 'Not Av.'; %default
                            
                            val(idx_ig(5)+1:idx_ig(6)-1)
                            disp ('Information NOT available (Stop_TS)')
                        end
                        
                        if (~isnan(val(idx_ig(7):idx_ig(8))))
                            self.a_evt(aux).comments=['pattern: ' (val(idx_ig(7):idx_ig(8)))...
                                ' | Classification: ' (val(idx_ig(8):idx_ig(9)))];
                        else
                            disp ('Information NOT available (Events comments)')
                        end

                        
                    else
                        break;
                    end%End of .evts interpretation
                end
                k=k+1;
                aux = aux+1;
            end
            
%             self.a_note_struct=self.a_note_struct(1:end-1);
            
            fclose(fd_ev);
            
            %% create objcts events
            
            
            for j = 1:length(self.a_evt)
                
                if datenum(self.a_evt(j).start_ts,'yyyy-mm-dd HH:MM:SS.FFF')<datenum(self.a_start_ts,'yyyy-mm-dd HH:MM:SS')...
                        || datenum(self.a_evt(j).start_ts,'yyyy-mm-dd HH:MM:SS.FFF')>datenum(self.a_stop_ts,'yyyy-mm-dd HH:MM:SS')%Verify is the event bellongs to the corrent mat file
                
                    continue;
                end
                    
                if strcmpi(self.a_evt(j).type,'seizure')
                    sample=floor(etime(datevec(self.a_evt(j).start_ts,'yyyy-mm-dd HH:MM:SS.FFF'),datevec(self.a_start_ts,'yyyy-mm-dd HH:MM:SS'))*self.a_samp_freq);
                    self.a_note_struct=[self.a_note_struct,struct('sample',sample,'note','EEG-ON')];
                    if ~strcmpi(self.a_evt(j).stop_ts,'Not Av.')
                        sample=floor(etime(datevec(self.a_evt(j).stop_ts,'yyyy-mm-dd HH:MM:SS.FFF'),datevec(self.a_start_ts,'yyyy-mm-dd HH:MM:SS'))*self.a_samp_freq);
                        self.a_note_struct=[self.a_note_struct,struct('sample',sample,'note','EEG-OFF')];
                    end
                else
                    sample=floor(etime(datevec(self.a_evt(j).start_ts,'yyyy-mm-dd HH:MM:SS.FFF'),datevec(self.a_start_ts,'yyyy-mm-dd HH:MM:SS'))*self.a_samp_freq);
                    self.a_note_struct=[self.a_note_struct,struct('sample',sample,'note',self.a_evt(j).type)];
                end
